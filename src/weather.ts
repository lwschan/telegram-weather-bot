...
import CurrentWeather from 'src/types/CurrentWeather';
...
export function kToF(k: number, precision = 0) {
  return round(((k - 273.15) * 9) / 5 + 32, precision);
}

// Kelvin to Celsius
export function kToC(k: number, precision = 0) {
  return round(k - 273.15, precision);
}

// m/s to mph
export function msToMph(speed: number, precision = 0) {
  // speed * seconds per hour / meters per mile
  return round((speed * 3600) / 1609.34, precision);
}

// m/s to kmh
export function msToKmh(speed: number, precision = 0) {
